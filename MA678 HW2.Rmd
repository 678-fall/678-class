...
inch_pct   <- 100 * b_h                        
# ≈ % change in earnings per +1 inch (small-beta approx)
male_ratio <- exp(b_m)                         
# multiplicative ratio male vs female at same height
male_pct   <- 100 * (male_ratio - 1)           
# % difference
cat(sprintf("Log model: +1 inch ≈ %.2f%% higher earnings; male vs female ≈ %.1f%% higher (same height).\n",
            inch_pct, male_pct))
set.seed(615)  # reproducible fold assignment (like p. 202 workflow)
k <- 10
n <- nrow(df)
folds <- sample(rep(1:k, length.out = n))

pred_raw  <- rep(NA_real_, n)
pred_logB <- rep(NA_real_, n)  # back-transformed log predictions with smearing
...
for (i in 1:k) {
  ...
  # Model B: log outcome + Duan smearing back-transform
  mB <- lm(log(earnk) ~ height + male, data = tr)
  smear <- mean(exp(residuals(mB)), na.rm = TRUE)  # E[exp(e)]
  muhat <- predict(mB, newdata = te)               # predictions on log scale
  pred_logB[folds == i] <- exp(muhat) * smear      # back to original scale
}

## ---- 4) Compare predictive performance on original scale ----
...
df_poll <- subset(df_poll, is.finite(mort) & mort > 0 &
                             is.finite(nox) & is.finite(so2) & is.finite(hc))

# Compare raw vs log outcome models
cv_res_poll <- cv_compare_outcome(
  mort ~ nox + so2 + hc,
  log(mort) ~ log(nox) + log(so2) + log(hc),
  data = df_poll,
  k = 10,
  seed = 615
)
